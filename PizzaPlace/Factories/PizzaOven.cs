...
namespace PizzaPlace.Factories
{
    public abstract class PizzaOven(TimeProvider timeProvider) : IPizzaOven
    {
            ...
            ComparableList<Stock> stock)
        {
            ...
            List<Pizza> readyPizzas = [];
            HashSet<Guid> waitingOrders = recipeOrders
            ...
            {
                (Pizza? readyPizza, Guid orderGuid) = await GetNextReadyPizza();
        ...
        protected Pizza GetPizza(PizzaRecipeType recipeType) =>
            recipeType switch
            {
                PizzaRecipeType.StandardPizza => new StandardPizza(),
                PizzaRecipeType.ExtremelyTastyPizza => new ExtremelyTastyPizza(),
                _ => throw new NotSupportedException(
                    $"Currently unable to make pizza for a recipe of type {recipeType}"),
            };
        ...
        {
            AddPizzaTasks();
            Task<(Pizza?, Guid)> donePizza = await Task.WhenAny(_pizzasInOven.Values);
        ...
        private bool AddPizzaTask()
        {
            lock (_ovenLock)
            {
                if (_pizzasInOven.Count < Capacity
                    && _pizzaQueue.TryDequeue(out (Func<Task<Pizza?>>, Guid) value))
                {
                    (Func<Task<Pizza?>> pizzaTaskCreator, Guid orderGuid) = value;
